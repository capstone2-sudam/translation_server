...
class SignLanguageCollateFn:
    ...
    def __call__(self, batch):
        ...
        """
        vision_seqs = [item[0] for item in batch]
        sensor_seqs = [item[1] for item in batch]
        ...
        padded_vision = pad_sequence(vision_seqs, batch_first=True, padding_value=0.0)
        padded_sensor = pad_sequence(sensor_seqs, batch_first=True, padding_value=0.0)

        # 2. 어텐션 마스크 (Attention Mask) 생성
        # 모델(Transformer)이 패딩된 0.0 값을 실제 데이터로 착각하지 않도록,
        # 실제 데이터가 있는 부분은 1, 패딩된 빈 공간은 0으로 표시하는 마스크를 만듭니다.
        seq_lengths = [len(seq) for seq in vision_seqs] # 원본 비디오들의 진짜/실제 프레임 길이가 몇 개였는 지 각각 기록 해두기
        # 패딩이 완료된 텐서의 최대 길이(Max_len)
        max_len = padded_vision.size(1)
        ...
        return {
            "vision_inputs": padded_vision,
            "sensor_inputs": padded_sensor,
